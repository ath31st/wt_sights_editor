...
Интерфейс на русском и английском. Номер версии виден в сайдбаре и на [странице релизов](https://github.com/ath31st/wt_sights_editor/releases).
...
Программа пишет только локальные файлы вашего аккаунта: прицелы в `UserSights` и назначение в `global.blk`. Никакие игровые файлы не затрагиваются и система anticheat не будет блокировать вас даже если программа будет запущена параллельно с игрой. Перед первой записью `global.blk` рядом сохраняется копия `global.blk.wtse.bak`. За свои файлы игры и за использование программы отвечаете вы.
